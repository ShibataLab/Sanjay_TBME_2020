...
%clear all; clc; close all;
 function [corr_tr,corr_ts,rmse_tr,rmse_ts,corr_et,rmse_et,r_tr,r_ts_et,oneSampleTime,net,idx]=  NN_PCA(dsfilt_musAct, lpfilt_trc_p23rot)
...
%%%% 1. NN Regression on normalized data
    ...
    net = fitnet(500);
    net = train(net, [ones(my,1) Y_ds_tr_n_proj_r]', Z_ds_tr_n_proj_r');
    %%%Regression
  Z_ds_tr_n_proj_r_pred = net([ones(my,1) Y_ds_tr_n_proj_r]')';
  tic;
  Z_ds_ts_n_proj_pred = net([ones(myts,1) Y_ds_ts_n_proj]')';  
  total_time = toc;
  Z_raw_n_proj_except_train_pred = net([ones(my_et,1) Y_raw_n_proj_except_train]')';
  Z_all_ts_n_proj_pred = net([ones(my_full_ts,1) Y_all_ts_n_proj]')';
